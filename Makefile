...
coverage: test ## run all the test with coverage
	$(eval __out_dir_coverage := ${__out_dir_debug}/coverage)
	$(eval __out_dir_html := ${__out_dir_coverage}/html)
	@mkdir -p ${__out_dir_html}
	# Generate our initial info
	@lcov -d "${__out_dir_debug}" -c -o "${__out_dir_coverage}/coverage.info"
...
qmakeDebug: ## generate a Makefile that is used to build the project
	$(eval __out_dir_debug := ${OUT_DIR}/debug)
	@${QMAKE} ${PRO_FILE} -o ${__out_dir_debug}/Makefile CONFIG+=debug CONFIG+=x86_64 CONFIG+=qml_debug CONFIG+=TEST
	@cd ${__out_dir_debug}/
	$(MAKE) -C ${__out_dir_debug} qmake_all

qmakeRelease: ## generate a Makefile that is used to build the project
	$(eval OUT_DIR := ${OUT_DIR}/__out_dir_release)
	@${QMAKE} ${PRO_FILE} -o ${__out_dir_release}/Makefile CONFIG+=x86_64 CONFIG+=qtquickcompiler
	@cd ${__out_dir_release}/
	$(MAKE) -C ${__out_dir_release} qmake_all

test: qmakeDebug ## run all the tests
	$(MAKE) -C ${__out_dir_debug} check
